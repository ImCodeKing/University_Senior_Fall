...
parser.add_argument('--ModelIdex', type=int, default=5500, help='which model to load')
        ...
        while total_steps < opt.Max_train_steps:
            ...
            while not done:
                ...
                if total_steps % opt.eval_interval == 0:
                    score = evaluate_policy(eval_env, agent, opt.max_action,
                                            turns=3)  # evaluate the policy for 3 times, and get averaged result
                    if opt.write: writer.add_scalar('ep_r', score, global_step=total_steps)
                    print('EnvName:', EnvName[opt.EnvIdex], 'seed:', opt.seed,
                          'steps: 2500+{}k'.format(int(total_steps / 1000)), 'score:', score)

                '''Save model'''
                if total_steps % opt.save_interval == 0:
                    agent.save(BrifEnvName[opt.EnvIdex], int(opt.ModelIdex + total_steps / 1000))
